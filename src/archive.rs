...
fn poll_next_raw<R: Read + Unpin>(
    ...
    loop {
        ...
        match futures_core::ready!(poll_try_read_all(&mut archive, cx, header.as_mut_bytes(), current_header_pos)) {
            Ok(true) => {}
    ...
    let mut remaining = entry.size;
    {
        ...
        if gnu.is_extended() {
            ...
            while ext.is_extended() {
                match futures_core::ready!(poll_try_read_all(&mut archive, cx, ext.as_mut_bytes(), current_ext_pos,))
                {
                    Ok(true) => {}
